from flask import Flask, render_template, \
    request, abort, redirect, url_for, session, make_response, g
...
# 用户登录页面
@app.route('/my/login', methods=['POST', 'GET'])
def login():
...
@app.route('/my/register')
def register():
    return render_template('register.html')


# 用户注册
@app.route('/my/register_check', methods=['POST', 'GET'])
def register_check():
    if request.method == 'POST':
...
@app.route('/my/success_register')
def success_register():
    return render_template('success_register.html')


# 查看收藏的知识卡片
@app.route('/my/collected_card', methods=['POST', 'GET'])
...
# 查看收藏的问题
@app.route('/my/collected_question', methods=['POST', 'GET'])
...
# 查看搜索过的知识卡片
@app.route('/my/looked_card', methods=['POST', 'GET'])
...
# 查看搜索过的问题
@app.route('/my/looked_question', methods=['POST', 'GET'])
...
class Config:
    """配置参数"""
    SQLALCHEMY_DATABASE_URI = 'mysql+pymysql://root:1121@127.0.0.1:3306/APP?charset=utf8mb4'
